...
from src.handlers.handler import get_group_users, get_or_create_iam_group
...
def test_get_group_users(iam_client, group_name, members):  
    client = _add_iam_groups(iam_client)  
    group = get_or_create_iam_group(group_name)
    users = get_group_users(group_name)

    assert len(users) == members
    assert users == group.get('Users', [])
